...
/**
 * Printed lorry receipt — `/print/lr/[tripId]` (part 05 §5.1, FSD B6, BR-17).
 *
 * Exactly four copies, labelled consignor · consignee · carrier · office —
 * the LR is the document of carriage: it travels with the goods and each
 * party keeps a countersigned copy, unlike the invoice (single copy — see
 * `/print/invoice/[invoiceId]`). Each copy carries the company GSTIN, PAN and
 * CIN from the control panel, consignor and consignee blocks, the goods
 * table, vehicle and driver, the six charge heads, terms, a Code 39 barcode
 * of the LR number and three signature blocks. The printed copy is the
 * primary form.
 */
const COPIES = ['Consignor copy', 'Consignee copy', 'Carrier copy', 'Office copy'];
    ...
    <div style={{ background: '#fff', color: '#000', padding: 20, fontFamily: 'var(--font-body)' }}>
      <div className="no-print" style={{ marginBottom: 14, display: 'flex', gap: 8 }}>
        ...
        <span className="muted" style={{ fontSize: 12, alignSelf: 'center' }}>
          Four copies · A4 · the printed copy is the primary form
        </span>
      </div>

      {COPIES.map((copy) => (
        <Sheet key={copy} label={copy} trip={trip} config={config} />
      ))}
    </div>
  );
}

function Sheet({ label, trip, config }: { label: string; trip: TripDetail; config: Config }) {
  const lr = trip.lr;
  ...
  return (
    <div className="sheet" style={{ maxWidth: 780, margin: '0 auto 22px', border: '1px solid #000', padding: 16 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', gap: 16, borderBottom: '2px solid #000', paddingBottom: 10 }}>
        ...
        <div style={{ textAlign: 'right' }}>
          <div style={{ fontSize: 11, letterSpacing: '.14em', textTransform: 'uppercase' }}>Lorry receipt · {label}</div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 12, paddingTop: 34, borderTop: '1px solid #000' }}>
        {['Consignor', 'For the carrier', 'Consignee'].map((sig) => (
          <div key={sig} style={{ borderTop: '1px solid #000', paddingTop: 4, fontSize: 10, textAlign: 'center' }}>
            {sig}
          </div>
        ))}
      </div>
